...
module.exports.handler = async function(event) {
  ...
  try {
    ...
    if (!tokenData.sessionToken) {
      return { statusCode: 400, body: JSON.stringify({ error: "No se pudo obtener sessionToken", data: tokenData }) };
    }
    ...
    const paymentResp = await fetch("https://ppp-test.safecharge.com/ppp/api/payment.do", {
      ...
      body: JSON.stringify({
        ...
        billingAddress: { firstName: "John", lastName: "Smith", email: "SCTest2@gmail.com", country: "US" },
        userDetails: { firstName: "John", lastName: "Smith", email: "SCTest2@gmail.com", country: "US" },
        timeStamp: timestamp,
    ...
    if (paymentData.status !== "SUCCESS") {
      return { statusCode: 400, body: JSON.stringify({ error: "Error en payment.do", data: paymentData }) };
    }

    return { statusCode: 200, body: JSON.stringify({ redirectUrl: paymentData.paymentOption?.redirectUrl || null }) };
